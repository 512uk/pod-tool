...
export default class PalletforcePodProcessor {
    ...
    async processTiffFiles(inFolderPath, archiveFolderPath) {
        ...
        for (const fileName of files) {
            ...
            const fileContents = await this.fileRepository.read(inFolderPath, fileName);
            ...
            await this.fileRepository.write(archiveFolderPath, _outputFile.name, _outputFile.content);

            // Upload the processed image file to the FTP server
            await this.ftpClient.connect();
            await this.ftpClient.upload(_outputFile.fullpath(), `/PWImages/InDel/${_outputFile.name}`);
